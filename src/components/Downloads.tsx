    ...
    <>
        ...
        <section className="grid min-h-[510px] grid-cols-[1.1fr_0.9fr] items-center gap-14 overflow-hidden px-[max(1.5rem,calc((100%_-_77.5rem)/2))] py-20 max-lg:grid-cols-1 max-lg:pt-17.5 max-lg:pb-0 max-md:px-6">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-4 max-[480px]:flex-col max-[480px]:items-stretch">
              <a className="inline-flex min-h-14 items-center justify-center gap-3 rounded-lg border-2 border-saffron bg-saffron px-6 font-bold text-navy no-underline transition duration-200 hover:-translate-y-0.5 max-[480px]:w-full" href={RELEASES_URL} target="_blank" rel="noreferrer">
                View Android releases
                <ArrowIcon />
              </a>
              <a className="inline-flex items-center gap-2.5 border-b border-white/70 px-1 py-3.5 font-bold text-white no-underline max-[480px]:w-full max-[480px]:justify-center" href={RELEASES_URL} target="_blank" rel="noreferrer">
                Firmware &amp; device images
                <ArrowIcon />
              </a>
            </div>
            <p className="mt-7.5 mb-0 text-[0.96rem] text-[#d6e8e7]">Releases are published on GitHub with stable and beta channels.</p>
          </div>
      ...
      <footer className="mx-auto grid min-h-47.5 w-full max-w-[82.5rem] grid-cols-[1fr_auto] items-center gap-12 px-6 max-md:grid-cols-1 max-md:px-4 max-md:py-13.5">
        ...
        <nav className="flex flex-wrap gap-7.5 max-[480px]:flex-col max-[480px]:gap-4" aria-label="Project links">
          ...
          <a className="text-navy underline decoration-1 underline-offset-4" href={RELEASES_URL} target="_blank" rel="noreferrer">App releases</a>
          <a className="text-navy underline decoration-1 underline-offset-4" href={RELEASES_URL} target="_blank" rel="noreferrer">Firmware releases</a>
        </nav>
      </footer>
    </>
  )
